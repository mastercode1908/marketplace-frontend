...
/**
 * TetHolidayBanner Component
 * Right banner with carousel functionality for Tet Holiday Sale
 * Auto-plays in reverse direction (from end to beginning)
 */
export default function TetHolidayBanner({ banners = [], currentSlide, onSlideChange, otherBannerSlide }) {
  const navigate = useNavigate();
  const [isRightBannerHovered, setIsRightBannerHovered] = useState(false);
  ...
  // Auto-play effect - chạy ngược từ cuối về đầu
  useEffect(() => {
    if (isAutoPlaying && heroSlides.length > 1) {
      carouselIntervalRef.current = setInterval(() => {
        // Chạy ngược: prevSlide thay vì nextSlide
        let next = (currentSlide - 1 + heroSlides.length) % heroSlides.length;
        // Ensure right banner is not same as left banner
        if (heroSlides.length > 1 && next === otherBannerSlide) {
          next = (next - 1 + heroSlides.length) % heroSlides.length;
        }
        onSlideChange(next);
  ...
  return (
    <div className="relative w-full h-[300px] rounded-2xl overflow-hidden hidden md:block">
      {heroSlides.length === 0 ? (
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            onMouseEnter={() => setIsRightBannerHovered(true)}
            onMouseLeave={() => setIsRightBannerHovered(false)}
            onClick={() => {
          ...
          >
            {/* Navigation Arrows for Right Banner */}
            {heroSlides.length > 1 && (
              <>
                <button
                  ...
                  onClick={prevSlide}
                  aria-label="Previous right slide"
                  style={{
                    opacity: isRightBannerHovered ? 1 : 0,
                    top: "50%",
                    transform: "translateY(-50%)",
                    transition: "opacity 0.3s ease",
                    pointerEvents: isRightBannerHovered ? "auto" : "none",
                    position: "absolute",
                  }}
                >
                  <LeftOutlined
      ...
      {/* Carousel Dots */}
      <div
        className="absolute bottom-4 right-1/4 transform translate-x-1/2 flex gap-2 z-10 hidden md:flex"
      ...
      >
        {heroSlides.map((_, index) => (
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            aria-label={`Go to right hero slide ${index + 1}`}
            animate={{
